...
def choose_move(board, big_board, forced_section):
    ...
    player_cells_corners = [s for s in SMALL_CORNERS if board[chosen_section + s] == current_player]
    ...
    if player_cells_corners:
        ...
        if surrounding_corners[i-1][0] in available_cells_corners for i in player_cells_corners:
            chosen_cell = surrounding_corners[i-1][0]
        elif surrounding_corners[i-1][1] in available_cells_corners for i in player_cells_corners:
